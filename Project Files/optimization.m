...
fmax = (1-b)*(1-out2(3)/Tomax)/(eff(4)*HVf/Cp1/Tomax - 1);

g(1) = x(3) - fmax; % f <= fmax
g(2) = (out2(3)+f*HVf/Cp1)/(1+f-b) - Tomax - CB*(b/.12)^.5; % Tb <= Tmax
g(3) = ST*1000-out2(1); % ST >= STdesired
if fab>0
    g(4) = (out2(4) + (f+fab)*HVf/Cp2)/(1+f+fab) - Tmax_ab; % Tab <= Tmaxab
    g(5) = x(4) - (1+fmax)*(Tmax_ab/out2(4) - 1)/((eff(7)*HVf/Cp2 - Tmax_ab)/out2(4)); % fab <= fmaxab
else
    g(4)=0;
    g(5)=0;
end
